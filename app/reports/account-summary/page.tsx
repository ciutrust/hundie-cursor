...
import { getAccountSummary } from "@/lib/queries/report-analytics";
...
export default async function AccountSummaryReportPage({ searchParams }: Props) {
  ...
  const [entities, rows] = await Promise.all([
    ...
    getAccountSummary(period, entitySlug),
  ]);
      ...
      <div className="overflow-hidden rounded-xl border border-border bg-card">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border">
            {rows.map((row) => (
              <tr key={`${row.entitySlug}-${row.accountSlug}`}>

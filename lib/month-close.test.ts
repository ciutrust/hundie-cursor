...
describe("cellStatus", () => {
  test("no activity -> empty", () => expect(cellStatus({ hasActivity: false, backlogCount: 0 })).toBe("empty"));
  test("activity, zero backlog -> closed (CPA-ready)", () =>
    expect(cellStatus({ hasActivity: true, backlogCount: 0 })).toBe("closed"));
  test("activity with backlog (unclassified or AMA) -> open", () =>
    expect(cellStatus({ hasActivity: true, backlogCount: 3 })).toBe("open"));
});

describe("rollupStatus (a month across entities, or an entity across months)", () => {
  test("nothing active -> empty", () =>
    expect(rollupStatus([{ hasActivity: false, backlogCount: 0 }])).toBe("empty"));
  test("any cell still has backlog -> open", () =>
    expect(
      rollupStatus([
        { hasActivity: true, backlogCount: 0 },
        { hasActivity: true, backlogCount: 2 },
      ]),
    ).toBe("open"));
  test("active and all cleared -> closed", () =>
    expect(
      rollupStatus([
        { hasActivity: true, backlogCount: 0 },
        { hasActivity: false, backlogCount: 0 },
      ]),
    ).toBe("closed"));
});
